...
1. **Dosya adı eşleşmesi** (yukarıdaki tablo) — en güvenilir yöntem, doğrudan kategoriye atar.
2. **İçerik bazlı yedek tanıma** — isim eşleşmezse, `.txt` uzantılı dosyalar için **içeriğin ilk satırına** bakılır:
...
1. Önce hedefin **wildcard response** verip vermediğini kontrol edin (rastgele bir path'e istek atıp 200 dönüp dönmediğine bakın).
2. Wildcard yoksa bile, path'lerin gerçekçi olup olmadığını (`%3f`, `%ff` gibi encode kalıntıları, mantıksız iç içe klasör isimleri) gözden geçirin.
3. Küçük gruplardaki (1-15 endpoint) bulgular genellikle daha güvenilirdir.
